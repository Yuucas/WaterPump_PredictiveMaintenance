...
if __name__ == "__main__":
    ...
    target_ts = TimeSeries.from_dataframe(dataset, value_cols=[target_column_name], time_col=time_column_name, fill_missing_dates=True, freq=None)
    covariates_ts = TimeSeries.from_dataframe(dataset, value_cols=covariate_cols, time_col=time_column_name, fill_missing_dates=True, freq=None)

    # Define split proportions
    train_frac = 0.7  # 70% for training
    val_frac = 0.15   # 15% for validation

    val_split_point = int(len(target_ts) * train_frac)
    test_split_point = int(len(target_ts) * (train_frac + val_frac))

    # Assign to global variables for Optuna's objective function
    train_target_ts = target_ts[:val_split_point]
    val_target_ts = target_ts[val_split_point:test_split_point]
    test_target_ts = target_ts[test_split_point:] # Hold-out test set

    train_covariates_ts_unscaled = covariates_ts[:val_split_point]
    val_covariates_ts_unscaled = covariates_ts[val_split_point:test_split_point]
    test_covariates_ts_unscaled = covariates_ts[test_split_point:]

    print(f"\nTraining series length: Target={len(train_target_ts)}, Covariates={len(train_covariates_ts_unscaled)}")
    print(f"Validation series length: Target={len(val_target_ts)}, Covariates={len(val_covariates_ts_unscaled)}")
    print(f"Test series length: Target={len(test_target_ts)}, Covariates={len(test_covariates_ts_unscaled)}")

    # --- 6. Scale Covariates ---
    # Scaler is fit ONLY on training covariates and then transforms train, val, and test.
    # Using Darts' Scaler wrapper around scikit-learn's StandardScaler
    ...
    test_past_covariates_scaled = covariate_scaler.transform(test_covariates_ts_unscaled) # For final test
    ...
    if len(test_target_ts) > 0:
        ...
        try:
            full_prediction_target = train_target_ts.append(val_target_ts)
            full_prediction_covariates = train_past_covariates.append(val_past_covariates)

            probabilistic_predictions = model_to_predict_with.predict(
                n=len(test_target_ts),
                series=full_prediction_target, # (train + val target)
                past_covariates=full_prediction_covariates, # (train + val covariates)
            )
